...
defmodule Ex02 do
  def new_counter(value \\ 0) do
    {:ok, counter_id} = Agent.start_link(fn -> value end)
    counter_id
  end

  def next_value(counter_id) do
    Agent.get_and_update(counter_id, fn value -> {value, value + 1} end)
  end
...
defmodule Test do
  ...
  test "counter using an agent" do
    ...
    # get_and_update allows me to both return the state and update the state in
    # one line which is quite rad. {value to return, new value}
